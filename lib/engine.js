...
async function transform({ prisma, data }) {
    ...
    for (const f of FORMS) {
        ...
        for (const r of rows) {
            ...
            // Per-row DB presence (e.g. OFT): is the parent already in the DB, and does it already
            // have its result report? Lets the UI offer a "results-only" import that never rewrites
            // the existing parent record.
            let db = null;
            if (f.resultStatus && !rec.__error && m.kvkId) {
                try { db = await f.resultStatus(prisma, rec, m.kvkId); } catch (e) { db = null; }
            }

            const row = { data: rec, _kvkId: m.kvkId, _kvkName: kvkName, _match: m.matched, _matchedName: m.kvkName, _issues: issues, _db: db };
            out.rows.push(row);
            out.counts.rows++;
...
async function commit({ prisma, forms }) {
    ...
    const report = { forms: [], totals: { inserted: 0, skipped: 0, resultsOnly: 0, failed: 0, noKvk: 0 } };
    ...
    for (const f of FORMS) { // catalogue order: Agri-Drone intro before its demonstrations
        ...
        const res = { sheet: f.sheet, label: f.label, model: f.model, inserted: 0, skipped: 0, resultsOnly: 0, failed: 0, noKvk: 0, failures: [] };
        ...
        for (const rawRec of inf.records) {
            ...
            try {
                ...
                // RESULTS-ONLY mode (e.g. OFT whose parent already exists but has no results yet):
                // never create or update the parent scalar record — just locate it and attach the
                // nested results. Skips FK/staff resolution and all scalar validation on purpose.
                if (rawRec._resultsOnly) {
                    if (!f.postCreate || !f.key) throw new Error('Results-only not supported for this form');
                    const where = {};
                    for (const [k, v] of Object.entries(f.key(d, kvkId))) where[k] = deISO(v);
                    // Prefer a duplicate that has NO results yet (the empty copy the user sees), so the
                    // results attach to the right record when several share the same key/title.
                    const parent = (f.resultsOnlyMatch
                        && await prisma[f.model].findFirst({ where: Object.assign({}, where, f.resultsOnlyMatch) }))
                        || await prisma[f.model].findFirst({ where });
                    if (!parent) throw new Error('Results-only: the matching record no longer exists in the DB — import the main record first');
                    await f.postCreate(prisma, parent, rawRec);
                    res.resultsOnly++;
                    continue;
                }
        ...
        report.totals.resultsOnly = (report.totals.resultsOnly || 0) + res.resultsOnly;
