...
// rows carry no mobile number, which the repo's required-mobile validation would throw on). The
// dual-year Natural-Farming models (beneficiaries/soil/financial) still get year+reportingYearDate
// filled automatically by the reporting-year bridge baked into the shared Prisma client.
//
// Where a scrape column has no matching column on the new model (e.g. a single "No. of participants"
// total when the model only stores gender/category counts), we stash the number in generalM and
// raise a row warning so nothing is silently dropped. These are flagged in the preview.
...
// scrape dates come as YYYY-MM-DD or D-M-YYYY; return a Date or null
const D = (v) => {
...
// reporting year (int) from any year-bearing column on the row; default 2025
...
// same, as a Jan-1 Date (for the non-dual-year models whose column is a DateTime)
const ryDate = (r, cols) => new Date(Date.UTC(ryYear(r, cols), 0, 1));

// stash a single total into generalM when the model only has gender/category breakdown columns
const stashTotal = (label) => (n) => (n ? [`${label} (${n}) stored under General-Male (model has no plain total column)`] : []);
    ...
    return [
        // ============================== NICRA ==============================
        {
            ...
            map: (r) => ({
                ...
                crop: S(r['Crop']), variety: S(r['Variety']), quantityQ: num(r['Quantity in (q)'] || r['Quantity in (q)']),
            }),
        ...
        },
        {
            ...
            warn: (r) => stashTotal('No. of farmers used Implement')(int(r['No. of farmers used Implement'])),
        },
        {
            ...
            warn: (r) => (int(r['VCRMC members (no.)']) ? [`VCRMC members total (${int(r['VCRMC members (no.)'])}) stored under Male-members (model splits male/female only)`] : []),
        },
        {
            ...
            warn: (r) => stashTotal('No. of farmers benefitted')(int(r['No. of farmers benefitted'])),
        },
        {
        ...
        },
        {
            ...
            map: (r) => ({
                dateOfVisit: D(r['Date of visited']) || ryDate(r), name: S(r['Name']),
                remark: S(r['VIP/Experts']), // stash the VIP/Expert type in remark (FK type not resolved)
            }),
        ...
        },
        {
            ...
            warn: (r) => (S(r['PI/CO PI']) ? [`PI/Co-PI type "${S(r['PI/CO PI'])}" not stored (model has no text field for it)`] : []),
        },
        {
            ...
            warn: (r) => stashTotal('No. of participants')(int(r['No. of participants'])),
        },
        ...
        },
        {
            ...
            warn: (r) => stashTotal('Participants')(int(r['Participants'])),
        },
        {
            sheet: 'Demonstration_Information', label: 'Natural Farming — Demonstration Info', model: 'demonstrationInfo', kvkCols: ['KVK Name', 'KVK'],
            map: (r) => ({
                reportingYear: ryDate(r), startDate: ryDate(r), endDate: ryDate(r),
                farmerName: S(r['Farmer Name']), villageName: '', address: '', contactNumber: '',
                gender: 'MALE', category: 'GENERAL', croppingPattern: '', farmingSituation: '',
                latitude: 0, longitude: 0,
                activityName: S(r['Name of Activity']), crop: S(r['Crop']), variety: S(r['Variety']),
                technologyDemonstrated: '', areaInHa: 0, farmerPracticeDetails: '', farmerFeedback: '',
            }),
            key: (d, kvkId) => ({ kvkId, farmerName: d.farmerName, activityName: d.activityName, crop: d.crop }),
        },
        {
            // "Farmer Already Practicing Natural Farming" — same model, distinct activityName
            sheet: 'Farmer_Details', label: 'Natural Farming — Farmer Already Practicing', model: 'demonstrationInfo', kvkCols: ['KVK Name', 'KVK'],
            map: (r) => ({
                reportingYear: ryDate(r), startDate: ryDate(r), endDate: ryDate(r),
                farmerName: S(r['Farmer Name']), villageName: '', address: S(r['Address']), contactNumber: '',
                gender: 'MALE', category: 'GENERAL', croppingPattern: S(r['Normal crops grown']), farmingSituation: '',
                latitude: 0, longitude: 0,
                activityName: 'Already Practicing Natural Farming', crop: S(r['Normal crops grown']), variety: '',
                technologyDemonstrated: '', areaInHa: 0,
                farmerPracticeDetails: S(r['Practicing year of natural farming']) ? `Practicing since ${S(r['Practicing year of natural farming'])}` : '',
                farmerFeedback: '',
            }),
        ...
        },
        {
            ...
            warn: (r) => (S(r['Name of Activity']) ? [`Activity "${S(r['Name of Activity'])}" not linked (activity-type FK not resolved in this tool)`] : []),
        },
