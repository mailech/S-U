...
function buildForms(ctx) {
    ...
    return [
        ...
        // result tables (columns/rows/cells), exactly as the new-site OFT Result form stores them.
        {
            ...
            // For results-only pushes, prefer a duplicate OFT that has NO result report yet, so the
            // results land on the (empty) copy the user is actually viewing — not the first match.
            resultsOnlyMatch: { resultReport: { is: null } },
            postCreate: oftPostCreate,
            // Per-row DB presence check (used by transform). Unique identity = kvkId + OFT title.
            // Returns whether the parent OFT already exists, and whether its result report exists,
            // so the UI can offer a "results-only" import that never rewrites the existing OFT.
            resultStatus: async (prisma, d, kvkId) => {
                if (!kvkId || !d.title) return { existsParent: false, existsResult: false };
                // There can be DUPLICATE OFTs with the same title. Look at all of them; the results
                // should go to a copy that has NO result report yet (the empty one the user sees).
                const all = await prisma.kvkoft.findMany({
                    where: { kvkId, title: d.title },
                    select: { kvkOftId: true, resultReport: { select: { oftResultReportId: true } } },
                });
                if (!all.length) return { existsParent: false, existsResult: false };
                const empty = all.find((o) => !o.resultReport);
                // does the incoming scrape actually carry result content to push?
                const oft = d._oft || {};
                const hasIncomingResult = !!(oft.report && (oft.report.finalRecommendation || oft.report.resultText)) || (oft.tables && oft.tables.length) || (oft.technologies && oft.technologies.length);
                // existsResult=false when at least one copy still needs results -> row becomes results-only.
                return { existsParent: true, existsResult: !empty, parentId: (empty || all[0]).kvkOftId, duplicates: all.length, hasIncomingResult };
            },
        },
    ];
}
